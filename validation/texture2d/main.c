...
#ifdef DCBENCH_BUILD
// The workload has to be identical in every pass, so everything that carries
// state across frames is reset here and the RNG is put back to a fixed seed.
// Without this, pass 3 measures different frames from pass 1 and the counts
// cannot be divided by each other.
static ElementBuffer *bench_buf;

static void bench_seed(void) {
	srand(12345);
	if (bench_buf != NULL) {
		bench_buf->at = 0;
		memset(bench_buf->scr, 0, sizeof(ScreenElement) * bench_buf->len);
	}
}
#endif
...
int main(int argc, char* argv[]) {
	...
	// Deliberately not set for the benchmark: a frame limiter spends the
	// slack in a wait loop, which lands in the counters and couples the
	// measurement to wall time.
	SetTargetFPS(60);
#endif
...
#ifdef DCBENCH_BUILD
	bench_buf = buf;
	dcbench_init("texture2d", bench_seed);
	while (dcbench_next_frame()) {
		dcbench_phase(0, "pick");
#else
