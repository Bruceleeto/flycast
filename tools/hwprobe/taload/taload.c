/* TA back-pressure test - derived from bruces_balls.
 *
 * A store queue flush measures 2.0 cycles of data-side freeze on the bench
 * (tools/hwprobe/sqsweep), at every spacing, to both destinations, and whether
 * the burst is 64 flushes or 2048. But bruces_balls' total freeze over its
 * total flushes is 3.6, leaving about 124,000 cycles a frame unaccounted for.
 *
 * Hypothesis: the difference is the tile accelerator pushing back. The bench
 * probe's TA is idle; this one rasterises 74,400 triangles into a 3MB vertex
 * buffer that ends up 83% full. When the TA cannot accept, the store queue
 * write stalls and the CPU waits.
 *
 * That predicts freeze-per-flush is LOAD DEPENDENT, so this renders a varying
 * number of balls and measures it at each. Geometry per ball is fixed, so the
 * flush count is exactly balls * VERTS_PER_BALL and needs no counting.
 *
 *   falls toward 2.0 as balls drop  -> back-pressure, hypothesis holds
 *   flat at 3.6 regardless          -> not load. The bench probe is
 *                                      unrepresentative some other way, and
...
//! Vertices submitted per ball: every stack is a strip of two vertices per
//! slice plus two more to close it. This is exactly the flush count per ball.
constexpr unsigned VERTS_PER_BALL = SPHERE_STACKS * (SPHERE_SLICES * 2 + 2);
...
/* Sin/cos pairs for every slice and stack angle, computed once at startup.

    The angles depend ONLY on the loop index - not on which ball is being drawn,
    and for slices not on which stack either - so the inner loop was computing
    the same 20 sin/cos pairs 1,767 times per frame (93 balls x 19 stacks).
    That single shz_sincosf() call was 27% of the frame on its own: the SH4
    pipeline profiler attributes 918,868 cycles per frame to the block holding
    it, out of a 3,333,333 cycle budget at 60fps.

    Each table is 168 bytes, so both stay resident in the operand cache, and the
    inner loop's angle math becomes a single load.

    Both are sized +1: the stack loop reads `stack` and `stack + 1`, and the
    strip's closing vertex uses slice index SPHERE_SLICES.
*/
static alignas(32) shz_sincos_t sliceTrig[SPHERE_SLICES + 1];
static alignas(32) shz_sincos_t stackTrig[SPHERE_STACKS + 1];

//! Fills in the trig tables above. Call once, before rendering anything.
static void init_trig_tables(void) {
    ...
    for(unsigned slice = 0; slice <= SPHERE_SLICES; slice++)
        sliceTrig[slice] = shz_sincosf(slice * sliceStep);

    for(unsigned stack = 0; stack <= SPHERE_STACKS; stack++)
        stackTrig[stack] = shz_sincosf(SHZ_F_PI_2 - stack * stackStep);
}
...
static void render_sphere(float radius, uint32_t base_color) {
    ...
    */
    ...
    for(unsigned stack = 0; stack < SPHERE_STACKS; stack++) {
        // Retrieve the sin/cos pairs for this stack and the next one. Both
        // depend only on the stack index, so they are table lookups instead of
        // two shz_sincosf() calls per stack per ball.
        shz_sincos_t sc1 = stackTrig[stack];
        shz_sincos_t sc2 = stackTrig[stack + 1];
        ...
        */
        ...
        for(unsigned slice = 0; slice < SPHERE_SLICES; slice++) {
            // The sin/cos pair for this slice. A function of `slice` alone,
            // so it is a table lookup - see init_trig_tables().
            shz_sincos_t sc_slice = sliceTrig[slice];
        ...
        {
            // The closing vertex of the strip, wrapping back around to the
            // start. Same table.
            shz_sincos_t sc_slice = sliceTrig[SPHERE_SLICES];
...
int main(int argc, const char *argv[]) {
    ...
    printf("# TA back-pressure test\n");
    printf("# flushes = balls * %u\n", VERTS_PER_BALL);
    printf("# freeze/flush should fall toward 2.0 if the extra cost is the TA\n");
    printf("#   pushing back rather than the flush itself.\n");
    printf("TAL\tballs\tflushes\tfrz_dc\tfrz_per_flush\tcycles\n");

    static const unsigned counts[] = { 93, 64, 32, 16, 8, 4, 2, 1 };

    for (unsigned ci = 0; ci < sizeof(counts) / sizeof(counts[0]); ci++) {
        const unsigned nballs = counts[ci];
        uint64 best_f = ~0ull, best_c = ~0ull;

        /* Several frames each, keeping the cheapest: an interrupt can only add. */
        for (int rep = 0; rep < 8; rep++) {
            perf_cntr_clear(PRFC0); perf_cntr_clear(PRFC1);
            perf_cntr_start(PRFC0, PMCR_ELAPSED_TIME_MODE, PMCR_COUNT_CPU_CYCLES);
            perf_cntr_start(PRFC1, PMCR_PIPELINE_FREEZE_BY_DCACHE_MISS_MODE,
            ...
            const uint64 c = perf_cntr_count(PRFC0);
            if (c < best_c) { best_c = c; best_f = perf_cntr_count(PRFC1); }
        }

        const double flushes = (double)nballs * VERTS_PER_BALL;
        printf("TAL\t%u\t%.0f\t%.0f\t%12.3f\t%.0f\n",
               nballs, flushes, (double)best_f,
               (double)best_f / flushes, (double)best_c);
    }
